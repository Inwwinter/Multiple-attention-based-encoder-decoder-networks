...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, text, is_train, batch_max_length):
        "Take in and process masked src and target sequences."
        memory = self.encode(src)
        out = self.decode(memory, text, is_train, batch_max_length)
        return memory, out
    
    def encode(self, src):
        # print(self.src_embed(src).size())
        return self.encoder(self.src_embed(src))
    
    def decode(self, memory, text, is_train, batch_max_length):
        return self.decoder(memory, text, is_train, batch_max_length)
...
def make_model(tgt_vocab, N=3, 
               d_model=256, d_ff=1024, d_feature=1024, h=8, dropout=0.1):
    "Helper: Construct a model from hyperparameters."

    resnet = models.resnet50(pretrained=False)
    pretrained_resnet = 'pretrained_models/resnet50.pth'
    ...
    model = EncoderDecoder(
        ...
        AttentionDecoder(d_model, d_model, tgt_vocab),
