...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        x=x.view(x.size(0),x.size(2),4,25)
        out = self.channel_attention(x) * x
        out = self.spatial_attention(out) * out
        out=out.contiguous().view(out.size(0),-1,out.size(1))
        return out
...
class ConvAttention(nn.Module):
    def __init__(self, channel):
        ...
        self.conv=nn.Conv1d(self.channel, self.channel, kernel_size=1)
    def forward(self, x):
...
class AttentionDecoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        #self.first_attention = nn.Conv2d(100,100,kernel_size=1)
        self.first_attention = CBAM(input_size)
        #self.first_attention=ConvAttention(input_size)
